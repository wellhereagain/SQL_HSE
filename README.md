...
- функции и примеры запросов с ними (functions.sql)
...
(проще всего прогнать следующую команду в паралельно открытом терминале без postgres):

```
psql -d insurance_company -f db_dump.sql;

```
Либо прогнать создание с нуля (далее все в изначальном терминале, с postgres):
